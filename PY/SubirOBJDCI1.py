...
# --- CONFIGURATION ---
FILE_PATH = r'G:\Mi unidad\CODIGOS\AUTOMATIZACIONES\EXCELABD\BaseDCIparatablerocontrolCapitaFamisanar2026.xlsx'  # Path to your Excel file
DB_CONFIG = {
...
def initial_load():
    ...
    rename_map = {
        'DCI FINAL 2': 'dcicod',
        'Descripcion Famisanar': 'descripcion',
        'Grupo Farmacológico': 'grpfarm',
        'Consumos promedio referencia': 'csmprmref',
        'Confirmar Tarifa Ramédicas': 'trfrmd',
        'Costo con tarifa actual': 'csttrfact'
    }
    df = df.rename(columns=rename_map)
    # Drop any phantom rows or "Total" rows from the bottom of the Excel file
    df = df.dropna(subset=['dcicod'])

    # 3. Data Type Correction (Crucial for SQL)
    # Ensure NITs are strings to prevent .0 or scientific notation
    df['dcicod'] = df['dcicod'].astype(str).str.replace('.0', '', regex=False)
    ...
    int_columns = ['csmprmref', 'trfrmd', 'csttrfact']
