USE Ramedicas;
     ...
     DECLARE @fecha_inicial_ini_kardex DATETIME = '2024-01-01 00:00:00'
...
--SELECT 
--CAST(SUM(Cantidades)as int) cantidad,
...
--FROM (
     SELECT T.TipNom "Tipo Comprobante",
          ...
          CASE WHEN F.FacEst = 'A' THEN 'Activo'
               WHEN F.FacEst = 'I' THEN 'Inactivo'
               WHEN F.FacEst = 'P' THEN 'Pendiente'
               WHEN F.FacEst = 'C' THEN 'Cancelado'
               WHEN F.FacEst = 'R' THEN 'Aprobado'
          END "Estado",
          CAST(F.FacFecTra AS DATE) "Fecha Origen Dispensacion",
          ...
          Pac.PacPriNom "Primer Nombre",
          Pac.PacSegNom "Segundo Nombre",
          Pac.PacPriApe "Primer Apellido",
          Pac.PacSegApe "Segundo Apellido",
          Pac.PacFec "Fecha Nacimiento",
          DATEDIFF(YEAR, Pac.PacFec, GETDATE()) - 
          CASE 
               WHEN MONTH(Pac.PacFec) > MONTH(GETDATE()) OR 
                    (MONTH(Pac.PacFec) = MONTH(GETDATE()) AND DAY(Pac.PacFec) > DAY(GETDATE()))
               THEN 1 
               ELSE 0 
          END "Edad Paciente",
          ...
          K.KarUni - K.KarUniCP "Diferencia",
          K.KarPrePub "Precio al Publico",
          0 "Descuento en valor",
          K.KarValTotMenDes + K.KarArtIva "Totalventa",
          K.KarPre "Costo",
          ...
          END "PBS/NO PBS",
          0 "Cantidad Documentos con Firma",
          ...
          K.KarUni "Cantidades",
          K.karvaltotMenDes "TotalVentasiVA",
          K.KarArtIva TotalIVA,
          K.KarFec  "Fecha Movimiento Kardex",
          COALESCE( T29.[FacSumDetDoc], 0) "Cantidad soportes",
          BSC.BodSucCCNom "Bodega origen"
     ...
     --LEFT JOIN Kardex Kar WITH (NOLOCK) ON Fac.FacSec = Kar.FacSec
     ...
     /*N.NitIde = '830003564' and CD.ConDisTip ='Cap' and *//*Fac.FacNro = 'PF62500002095' and*/
