...
DECLARE @fecha_inicial_ini_kardex DATETIME = null
DECLARE @fecha_inicial_fin_kardex   DATETIME = '20260428'-- esta sirve para validar entrega de pendientes
DECLARE @fecha_inicial_dispensacion DATETIME = null;
DECLARE @fecha_final_dispensacion DATETIME = null; --

SELECT 
T.TipNom "Tipo Comprobante",
       S.SucNom "Sucursal",
       F.FacNro "Documento",
...
LEFT JOIN Factura Fac WITH (NOLOCK) ON F.FacNro = Fac.FacEscPac
...
WHERE /*f.facnitsec IN ('144143','4322') and*/ f.facest= 'A'  AND F.FacNro = 'RM52600003738'
 and ((@fecha_inicial_origen is not null and @fecha_final_origen is not null and F.FacFecTra BETWEEN @fecha_inicial_origen AND @fecha_final_origen)
...
--1 Ejemplo Usuario creacion en blanco

F.FacNro IN ('PF22600000312')

--2 Ejemplo Tipo de documento de paciente esta en blanco

F.FacNro IN ('PF22600002654', 'PF12600059028', 'PF12600059028')

--3 Ejempo Nombre de paciente esta en blanco 4. Igualmente para el regimen del paciente // queda validar si todo es compensar

F.FacNro IN ('P152602003566', 'P15002813', 'P16009077')

--5 Ejemplo Tipo de servicio en blanco 6. Fecha formula null 1753-01-01 // 27 registros queda validar si todo es compensar

F.FacNro IN ('P15049', 'P16109', 'P16074')

--6 Ejemplo Codigo Ciudad y Nombre Ciudad ANTOFAGASTA Chile

F.FacNro IN ('P142600042721', 'P142600039096', 'PF42600054557')

--7 Genero Paciente en blanco 

F.FacNro IN ('P152602002991', 'P152602003786')

--8 Codigo Medico  y nombre blanco

F.FacNro IN ('P15003167') 

--9 Ips codigo y nombre en blanco

F.FacNro IN ('PF72500000170', 'PF72500000170', 'PAF2500018025', 'PF72500004968')

--10 Cohorte en blanco

F.FacNro IN ('PAF2500026913', 'PAF2500014796', 'PF12500034450')

--11 Grupo en blanco (LAB)

F.FacNro IN ('P122500013845', 'P142500014385', 'P132600031219')

--12 Cantidad preescrita negativo -960

F.FacNro IN ('RC12500056779')

--13 Producto Acido fucidico tiene un precio publico y total venta raro

F.FacNro IN ('RM626001964', 'RL22400005110')

--14 Tipo de servicio

F.FacNro IN ('PF72500051277', 'PF12600054666')

--14 Precio Publico  0

F.FacNro IN ('PAF2600034053')

--14 Precio Publico  -1

F.FacNro IN ('RM626001964', 'RM626002593')

-- Precio 1

F.FacNro IN ('RC12500035782', 'RL22500031781')